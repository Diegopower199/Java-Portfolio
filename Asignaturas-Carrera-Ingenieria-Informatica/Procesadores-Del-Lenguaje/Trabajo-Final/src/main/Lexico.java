...
public class Lexico {
    ...
    public Lexico(String programa) {
        this.posicion = 0;
        this.lineas = 1;
        this.componentesLexicos = new ComponentesLexicos("resources/lexemas.txt");
    ...
    public ComponenteLexico getComponenteLexico() {
        ...
        Boolean comentarioLineaActivo = false;
        Boolean comentarioBloqueActivo = false;
        Boolean caracterDeLexemaLeido = false;
        ...
        do {
            ...
            if (this.caracter == 0)
                ...
                this.lineas++;
                comentarioLineaActivo = false;
                caracterDeLexemaLeido = false;
            } else if (this.caracter == '/') {
                this.caracter = extraeCaracter();

                if (this.caracter == '/') {
                    comentarioLineaActivo = true;
                } else if (this.caracter == '*') {
                    comentarioBloqueActivo = true;
                } else {
                    devuelveCaracter();
                    this.caracter = '/';
                    caracterDeLexemaLeido = true;
                }
            } else if (comentarioBloqueActivo && this.caracter == '*') {
                this.caracter = extraeCaracter();

                if (this.caracter == '/') {
                    comentarioBloqueActivo = false;
                    caracterDeLexemaLeido = false;
                } else {
                    devuelveCaracter();
                }
                ...
                caracterDeLexemaLeido = true;
        } while (comentarioLineaActivo || comentarioBloqueActivo || !caracterDeLexemaLeido);
